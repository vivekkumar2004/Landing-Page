...
export default function ContactForm() {
  ...
  const [formData, setFormData] = useState({
    name: '', email: '', phone: '', currentRole: '', experience: '', intention: '', pathway: ''
  });
    ...
    <section id="contact" className="bg-zinc-950 text-white py-24 px-4 md:px-8 border-t border-zinc-900 scroll-mt-20">
      <div className="max-w-xl mx-auto space-y-8">
        ...
        <div className="text-center space-y-3">
          <div className="inline-flex items-center gap-1.5 bg-amber-500/10 border border-amber-500/20 px-3 py-1 rounded-full text-[10px] uppercase tracking-widest text-amber-500 font-medium">
            {step === 0 ? "Direct Institutional Registration" : step === 4 ? "Dossier Locking Phase" : "Eligibility Verification"}
          </div>
          <h2 className="text-3xl font-serif font-medium tracking-wide">
            {step === 0 ? "Start With a Conversation" : step === 4 ? "Review & Confirm" : "Map Your Criteria"}
          </h2>
        </div>

        <div className="flex gap-2 max-w-xs mx-auto">
          {[0, 1, 2, 3, 4].map((i) => (
            <div key={i} className={`h-1 flex-1 rounded-full transition-all duration-300 ${i <= step ? 'bg-amber-500' : 'bg-zinc-800'}`} />
          ))}
        </div>
        ...
        <Card className="p-6 md:p-8 min-h-[380px] flex flex-col justify-center">
          ...
          {step === 0 && (
            <form onSubmit={handleInitialSubmit} className="space-y-4 animate-fadeIn">
              <div className="space-y-1.5">
                ...
                <input type="text" required name="name" value={formData.name} onChange={handleInputChange} className="w-full bg-zinc-950 border border-zinc-800/80 rounded-lg p-3 text-sm text-zinc-200 focus:outline-none focus:border-amber-500/50 transition-colors" placeholder="e.g. Dr. Vivek Verma" />
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-xs uppercase tracking-wider text-zinc-400 font-medium">Email Address</label>
                  <input type="email" required name="email" value={formData.email} onChange={handleInputChange} className="w-full bg-zinc-950 border border-zinc-800/80 rounded-lg p-3 text-sm text-zinc-200 focus:outline-none focus:border-amber-500/50 transition-colors" placeholder="name@company.com" />
                </div>
                <div className="space-y-1.5">
                  <label className="text-xs uppercase tracking-wider text-zinc-400 font-medium">Phone Number</label>
                  <input type="tel" required name="phone" value={formData.phone} onChange={handleInputChange} className="w-full bg-zinc-950 border border-zinc-800/80 rounded-lg p-3 text-sm text-zinc-200 focus:outline-none focus:border-amber-500/50 transition-colors" placeholder="+91 98765 43210" />
                </div>
              </div>
              <div className="space-y-1.5">
            ...
            <div className="space-y-6 animate-fadeIn">
              ...
              <div className="grid gap-3">
                {stepsData[step].options.map((option, idx) => {
                  ...
                  return (
                    <button key={idx} type="button" onClick={() => handleOptionSelect(stepsData[step].key, option)} className={`w-full cursor-pointer text-left px-5 py-3.5 rounded-lg border text-xs transition-all duration-300 flex items-center justify-between group ${isSelected ? 'border-amber-500 bg-amber-500/5 text-amber-400' : 'border-zinc-800/80 bg-zinc-950/50 text-zinc-400 hover:border-zinc-700 hover:bg-zinc-950 hover:text-zinc-200'}`}>
                      <span className="font-light tracking-wide max-w-[90%] leading-normal">{option}</span>
                      <div className={`w-3.5 h-3.5 rounded-full border flex items-center justify-center shrink-0 transition-colors ${isSelected ? 'border-amber-500 bg-amber-500' : 'border-zinc-700 group-hover:border-zinc-500'}`}>
                        {isSelected && <div className="w-1 h-1 bg-zinc-950 rounded-full" />}
                      </div>
                    </button>
                  );
                })}
          ...
          {step === 4 && (
            <div className="space-y-6 animate-fadeIn text-center py-4">
              <div className="w-12 h-12 bg-amber-500/10 border border-amber-500/30 rounded-full flex items-center justify-center mx-auto text-amber-500 text-xl">✓</div>
              <div className="space-y-1.5">
